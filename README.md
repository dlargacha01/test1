...
types, one for our`non_keepers` and one for our `goalkeepers`, and lastly a
...
compare two different players. We can see the composite schemas below:
...
We then included these in our old table schemas as follows:
...
Here are the two triggers we have created:
...
Notice that we could just as well run `DELETE FROM employees WHERE club_id = 100`
where the team would be deleted because of no employees rather than no players.
<br>
Will output:
